...
class TransaksiSeminarController extends Controller {  
    /**
     * daftar komponen biaya
     */
    public function index(Request $request)
    {
        $this->hasPermissionTo('KEUANGAN-TRANSAKSI-SEMINAR_BROWSE');
        ...
        if ($this->hasRole(['mahasiswa', 'mahasiswabaru']))
        {
            ...
            $daftar_transaksi = TransaksiDetailModel::select(\DB::raw('
                                                    ...
                                                    ->where('pe3_transaksi_detail.kombi_id',402)        
    ...
    {
        $this->hasPermissionTo('KEUANGAN-TRANSAKSI-SEMINAR_STORE');
        ...
        try 
        {
            ...
            if (!($biaya_kombi > 0))
            {
                throw new Exception ("Komponen Biaya SEMINAR (402) belum disetting pada TA $tahun");  
            }

            $transaksi = \DB::transaction(function () use ($request,$mahasiswa,$biaya_kombi) {
                $no_transaksi='402'.date('YmdHms');
                ...
                $transaksi->desc='SEMINAR ' . $request->input('ta').$request->input('semester_akademik');
                $transaksi->save();
            ...
            return Response()->json([
                                        ...
                                        'pid' => 'store',       
                                        'transaksi' => $transaksi,
                                        'message' => 'Transaksi SEMINAR berhasil di input.'
                                    ], 200); 
        }
        catch (Exception $e)
        {
    ...
    {
        $this->hasPermissionTo('KEUANGAN-TRANSAKSI-SEMINAR_DESTROY');
        ...
        if (is_null($transaksi))
        {
            return Response()->json([
                                    ...
                                    'pid' => 'destroy',           
                                    'transaksi' => $transaksi,     
                                    'message' => ["Transaksi seminar dengan ($id) gagal dihapus"]
                                ], 422); 
        }
        else if ($transaksi->status==0)
        {
            \App\Models\System\ActivityLog::log($request,[
                                                            ...
                                                            'message' => 'Menghapus transaksi seminar dengan id (' . $id. ') berhasil'
                                                        ]);
            $transaksi->delete();
            return Response()->json([
                                        'status' => 1,
                                        'pid' => 'destroy',    
                                        'message' => "transaksi seminar dengan id ($id) berhasil dihapus"
                                    ], 200);    
        }
        else
        {
