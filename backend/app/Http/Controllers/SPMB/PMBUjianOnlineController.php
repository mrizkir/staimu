...
class PMBUjianOnlineController extends Controller {  
	...
	{   
		$peserta = PesertaUjianPMBModel::select(\DB::raw('
				...
				pe3_formulir_pendaftaran.isdulang
			'))
			->join('pe3_formulir_pendaftaran', 'pe3_peserta_ujian_pmb.user_id', 'pe3_formulir_pendaftaran.user_id')
			->where('pe3_peserta_ujian_pmb.user_id', $id)
			->first();
