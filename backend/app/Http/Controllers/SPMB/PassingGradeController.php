...
use App\Models\SPMB\PassingGradeModel;
...
class PassingGradeController extends Controller {  
    ...
    {
        $this->hasPermissionTo('SPMB-PMB-PASSING-GRADE_SHOW');

        $soal=PassingGradeModel::find($id);
        if (is_null($soal))
        {
            return Response()->json([
                                    'status'=>1,
    ...
    {
        $this->hasPermissionTo('SPMB-PMB-PASSING-GRADE_UPDATE');
        ...
        else
        {
            ...
            $jawaban = PassingGradeModel::find($request->input('jawaban_benar'));
            if (!is_null($jawaban))
            {
                $jawaban->status=1;
                $jawaban->save();
    ...
    { 
        $this->hasPermissionTo('SPMB-PMB-PASSING-GRADE_DESTROY');
