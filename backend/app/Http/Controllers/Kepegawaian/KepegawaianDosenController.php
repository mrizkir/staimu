    ...
    else
    {
      ...
      $this->validate($request, [
        'nidn' => [
          'required',
          'numeric',
          Rule::unique('pe3_dosen')->ignore($user_dosen->nidn, 'nidn')->where(function($query) use ($user_dosen) {
            $query->where('user_id', '<>', $user_dosen->user_id);
          }),
        ],
        'nipy' => [
          'required',
          'numeric',
          Rule::unique('pe3_dosen')->ignore($user_dosen->nipy, 'nipy')->where(function($query) use ($user_dosen) {
            $query->where('user_id', '<>', $user_dosen->user_id);
          }),
        ],
        'name' => 'required',
      ...
      $user = \DB::transaction(function () use ($request, $user, $user_dosen) {
