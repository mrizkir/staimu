...
class UjianMunaqasahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($this->hasRole(['superadmin','akademik','programstudi','puslahta']))
        {
            ...
            $daftar_ujian = KRSModel::select(\DB::raw('
                                    pe3_krs.id,
                                    pe3_krs.nim,
            ...
            {
                $daftar_ujian=$daftar_ujian->whereRaw('(pe3_krs.nim LIKE \''.$request->input('search').'%\' OR pe3_formulir_pendaftaran.nama_mhs LIKE \'%'.$request->input('search').'%\')')                                                    
                            ->orderBy('tasmt','desc')
                            ->get();
            }            
            else
            {
                $daftar_ujian=$daftar_ujian->where('pe3_krs.kjur',$prodi_id)
                                        ->where('pe3_krs.tahun',$ta)
                                        ->where('pe3_krs.idsmt',$semester_akademik)                            
                                        ->get();
            }
            $daftar_ujian->transform(function ($item,$key) {                
    ...
    { 
        $this->hasPermissionTo('AKADEMIK-PERKULIAHAN-UJIAN-MUNAQASAH_DESTROY');
