...
class DulangMahasiswaNonAktifController extends Controller 
{
	/**
	 * daftar mahasiswa
	 */
	...
	{
		$this->hasPermissionTo('AKADEMIK-DULANG-NON-AKTIF_BROWSE');
	...
	public function cek(Request $request)
	{
		...
		try
		{
			...
			if ($profil->k_status == 'L' || $profil->k_status == 'K' || $profil->k_status == 'D' || $profil->k_status == 'N')
			{
  ...
  public function massal(Request $request)
  {
		...
		$dulang = \DB::transaction(function () use ($request) {
  ...
  public function store(Request $request)
  {
		...
		try 
		{
			...
			if ($data_mhs->k_status == 'L' || $data_mhs->k_status == 'K' || $data_mhs->k_status == 'D' || $data_mhs->k_status == 'N')
			{
	...
	{ 
		$this->hasPermissionTo('AKADEMIK-DULANG-NON-AKTIF_DESTROY');
