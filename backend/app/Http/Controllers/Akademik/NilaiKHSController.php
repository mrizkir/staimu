  ...
  public function printpdf(Request $request,$id)
  {
    ...
    else
    {
      ...
      {            
        ...
        $fakultas = ProgramStudiModel::find($krs->kjur)->fakultas;
        $dekan = json_decode($fakultas->config, true);
        
        $data_dekan['nama_dekan'] = '';
        if(isset($dekan['data_dekan']['name']))
        {
          $data_dekan['nama_dekan'] = $dekan['data_dekan']['name'];
        }
        $data_dekan['nidn'] = '';
        if(isset($dekan['data_dekan']['nidn']))
        {
          $data_dekan['nidn'] = $dekan['data_dekan']['nidn'];
        }
        $data_dekan['jabatan_fungsional'] = '';
        if(isset($dekan['data_dekan']['jabatan_fungsional']))
        {
          $data_dekan['jabatan_fungsional'] = $dekan['data_dekan']['jabatan_fungsional'];
        }
        
        $headers=[
        'HEADER_1' => $config['HEADER_1'],
        'HEADER_2' => $config['HEADER_2'],
        'HEADER_3' => 'FAKULTAS '. strtoupper($fakultas->nama_fakultas),
