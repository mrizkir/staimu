		...
		<v-container fluid v-if="Object.keys(datakrs).length">
			<v-row>
				<v-col cols="12">
					<DataKRS
						:datakrs="datakrs"
						url="/akademik/perkuliahan/krs/daftar"
...
<script>
	...
	export default {
		...
		methods: {
			async fetchKRS() {
				await this.$ajax
					.get("/akademik/perkuliahan/krs/" + this.krs_id, {
						headers: {
					...
					await this.$ajax
						.post(
							"/akademik/perkuliahan/pembagiankelas/storepeserta",
							{
								...
								members_selected: JSON.stringify(
									Object.assign({}, members_selected)
								),
								pid: "krs",
							},
							{
				...
				this.$root.$confirm
					.open(
						...
						"Apakah Anda ingin menghapus matakuliah (" + item.nmatkul + ") ?",
						{
							...
							width: 600,
							desc:
								"proses ini juga menghapus seluruh data yang terkait dengan matkul ini.",
						}
					)
					.then(confirm => {
						if (confirm) {
							...
							this.$ajax
								.post(
									"/akademik/perkuliahan/krs/deletematkul/" + item.id,
									{
										_method: "DELETE",
									},
									{
			...
			async printpdf() {
				this.btnLoading = true;
				await this.$ajax
					.get("/akademik/perkuliahan/krs/printpdf/" + this.krs_id, {
						headers: {
							Authorization: this.$store.getters["auth/Token"],
						},
					})
