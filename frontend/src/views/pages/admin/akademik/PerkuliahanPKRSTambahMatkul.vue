<template>
	<AkademikLayout :showrightsidebar="false">
		...
		<v-container fluid v-if="Object.keys(datakrs).length">
			<v-row>
				<v-col cols="12">
					<DataKRS :datakrs="datakrs" :url="'/akademik/perkuliahan/pkrs/'+datakrs.id+'/detail'" :totalmatkul="totalMatkul" :totalsks="totalSKS" />
				</v-col>
			</v-row>
			<v-row>
				<v-col cols="12">
...
export default {
	...
	methods: {
		async fetchKRS()
		{
			...
			if (Object.keys(this.datakrs).length)
			{
				...
				await this.$ajax.post("/akademik/perkuliahan/pkrs/penyelenggaraan",
				{
	...
	computed: {
		...
		totalMatkul() {			
			return this.daftar_matkul_selected.length;
		},
   },
	components: {
		AkademikLayout,
