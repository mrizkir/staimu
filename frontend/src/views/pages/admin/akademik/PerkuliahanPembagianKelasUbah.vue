		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman untuk melakukan penambahan kelas pada tahun akademik dan
					semester terpilih.
				</v-alert>
			</template>
		</ModuleHeader>
						...
						<v-card class="mb-2">
							...
							<v-card-actions>
								...
								<v-btn
									...
									text
									@click.stop="
										$router.push('/akademik/perkuliahan/pembagiankelas/daftar')
									"
								>
									KEMBALI
								</v-btn>
	...
	export default {
		...
		data: () => ({
			...
			rule_matakuliah: [
				value =>
					!!value ||
					"Mohon dipilih matakuliah yang diselenggaran untuk dosen pengampu ini!!!",
			],
			rule_kelas: [value => !!value || "Mohon dipilih kelas matakuliah ini!!!"],
		...
		methods: {
			initialize: async function() {
				...
				await this.$ajax
					.get(process.env.VUE_APP_API_HOST + "/h2h/zoom", {
						headers: {
							Authorization: this.$store.getters["auth/Token"],
						},
					})
					...
					await this.$ajax
						.post(
							"/akademik/perkuliahan/pembagiankelas/" + this.formdata.id,
							{
								...
								penyelenggaraan_dosen_id: JSON.stringify(
									Object.assign({}, this.formdata.penyelenggaraan_dosen_id)
								),
		...
		watch: {
			async dosen_id(val) {
				await this.$ajax
					.post(
						"/akademik/perkuliahan/penyelenggaraanmatakuliah/matakuliah",
						{
							user_id: val,
