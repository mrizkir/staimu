		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman untuk melihat daftar mahasiswa yang keluar.
				</v-alert>
			</template>
		</ModuleHeader>
		...
		methods: {
			...
			save: async function() {
				if (this.$refs.frmdata.validate()) {
					...
					await this.$ajax
						.post(
							"/akademik/dulang/mhskeluar/store",
							{
				...
				this.$root.$confirm
					...
					.then(confirm => {
						if (confirm) {
							...
							this.$ajax
								.post(
									"/akademik/dulang/mhskeluar/" + item.id,
									{
										_method: "DELETE",
									},
									{
