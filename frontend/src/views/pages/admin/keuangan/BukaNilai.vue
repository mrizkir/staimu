		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman untuk membuka akses terhadap nilai KHS dan transkrip per tahun akademik,
					dan semester.
				</v-alert>
			</template>
		</ModuleHeader>
					...
					>
						...
						<template v-slot:item.actions="props">
							<v-switch
								...
								class="font-weight-bold"
                @change="update(props.item)"
							>
							</v-switch>
              <v-icon v-if="props.item.locked">
                mdi-eye
              </v-icon>
              <v-icon v-else>
                mdi-eye-off
	...
	export default {
      ...
      this.initialize();			
		...
		methods: {
			...
			update(item) {
        var pid = item.locked == true ? "MEMBUKA" : "MENUTUP";
				this.btnLoadingTable = true;
				this.$root.$confirm
					.open(
						"Delete",
						"Apakah Anda ingin " + pid + " akses terhadap nilai mahasiswa dengan NIM (" + item.nim + ") ?",
						{
							color: "red",
							width: 600,							
						}
					...
					.then(confirm => {
						if (confirm) {
							...
							var editedIndex = this.datatable.indexOf(item);
							this.datatable[editedIndex].locked = !item.locked;							
						}
					});
			},
			async printpdf(item) {
