<template>
	<KeuanganLayout>
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman ini berisi informasi konfirmasi pembayaran mahasiswa, silahkan
					melakukan filter tahun akademik.
				</v-alert>
			</template>
		</ModuleHeader>
		<v-container fluid>
											...
											<v-row>
												<v-col xs="12" sm="6" md="6">
													<v-card flat>
														...
														<v-card-subtitle>
															{{
																$date(data_transaksi.tanggal).format(
																	"DD/MM/YYYY"
																)
															}}
															{{
								...
								>
									<v-card color="grey lighten-4">
										<v-toolbar elevation="2">
											...
											<v-divider class="mx-4" inset vertical></v-divider>
											<v-spacer></v-spacer>
											<v-icon @click.stop="closedialogdetailitem()">
												mdi-close-thick
											</v-icon>
										</v-toolbar>
										<v-card-text>
											...
											<v-row>
												...
												<v-col xs="12" sm="6" md="6">
													<v-card flat>
														...
														<v-card-subtitle>
															{{
																$date(data_konfirmasi.tanggal_bayar).format(
																	"DD/MM/YYYY"
																)
															}}
														</v-card-subtitle>
							...
							<td :colspan="headers.length" class="text-center">
								<v-col cols="12">
									...
									{{
										item.updated_at_konfirm == "N.A"
											? "N.A"
											: $date(item.updated_at_konfirm).format("DD/MM/YYYY HH:mm")
									}}
								</v-col>
								<v-col
									cols="12"
								...
								>
									<v-btn
										...
										class="mb-2"
										:disabled="
											(item.status_konfirmasi == 'UNVERIFIED' ? false : true) ||
												btnLoading
										"
									>
										VERIFIKASI
									...
									<v-btn
										...
										class="mb-2"
										:disabled="
											(item.nama_status == 'PAID' ? true : false) || btnLoading
										"
									>
										BATALKAN
									</v-btn>
		...
		<template v-slot:filtersidebar>
			<Filter18
				v-on:changeTahunPendaftaran="changeTahunAkademik"
				v-on:changeProdi="changeProdi"
				ref="filter18"
			/>
		</template>
		<dialog-printout
			pid="konfirmasipembayaran"
			title="Daftar Konfirmasi Pembayaran"
			ref="dialogprint"
		>
		</dialog-printout>
	</KeuanganLayout>
	...
	export default {
		...
		data: () => ({
			...
			rule_channel_pembayaran: [
				value =>
					!!value || "Mohon dipilih Channel Pembayaran mohon untuk dipilih !!!",
			],
			rule_nama_pengirim: [value => !!value || "Mohon diisi nama pengirim !!!"],
			rule_nomor_rekening: [
				value => !!value || "Mohon diisi nomor rekening pengirim !!!",
				value => /^[0-9]+$/.test(value) || "Nomor Rekening hanya boleh angka",
			],
			rule_nama_bank: [value => !!value || "Mohon diisi nama bank !!!"],
			rule_tanggal_bayar: [
		...
		methods: {
			...
			async addItem(item) {
				await this.$ajax
					.get("/keuangan/channelpembayaran", {
						headers: {
							Authorization: this.$store.getters["auth/Token"],
						},
					})
			...
			async viewItem(item) {
				await this.$ajax
					.get("/keuangan/konfirmasipembayaran/" + item.id, {
						headers: {
							Authorization: this.$store.getters["auth/Token"],
						},
					})
			...
			save() {
				if (this.$refs.frmdata.validate()) {
					...
					data.append(
						"nomor_rekening_pengirim",
						this.formdata.nomor_rekening_pengirim
					);
					data.append(
						"nama_rekening_pengirim",
						this.formdata.nama_rekening_pengirim
					);
					...
					this.$ajax
						.post("/keuangan/konfirmasipembayaran/store", data, {
							headers: {
								Authorization: this.$store.getters["auth/Token"],
			...
			async verifikasi(item) {
				this.$root.$confirm
					.open(
						"Konfirmasi Pembayaran",
						"Apakah sudah benar data bukti bayar kode billing " +
							item.no_transaksi +
							" ?",
						{ color: "primary" }
					)
					.then(confirm => {
						if (confirm) {
							...
							this.$ajax
								.post(
									"/keuangan/transaksi/verifikasi/" + item.id,
									{
										_method: "put",
									},
									{
			...
			async cancel(item) {
				this.$root.$confirm
					.open(
						"Batalkan Transaksi",
						"Apakah Anda ingin membatalkan transaksi dengan kode billing " +
							item.no_transaksi +
							" ?",
						{ color: "red" }
					)
