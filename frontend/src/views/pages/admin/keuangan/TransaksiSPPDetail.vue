	...
	<KeuanganLayout :showrightsidebar="false">
		<ModuleHeader>
			...
			<template v-slot:subtitle v-if="data_transaksi">
				TAHUN AKADEMIK / SEMESTER TRANSAKSI: {{ data_transaksi.ta }} -
				{{ $store.getters["uiadmin/getNamaSemester"](data_transaksi.idsmt) }}
			</template>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman ini digunakan untuk melakukan transaksi SPP mahasiswa baru dan
					lama.
				</v-alert>
			</template>
		</ModuleHeader>
				...
				<v-col cols="12">
					<v-card>
						<v-card-title>
							...
							<v-spacer></v-spacer>
							<v-icon @click.stop="closeDetailTransaksi">
								mdi-close-thick
							</v-icon>
						</v-card-title>
						<v-card-text>
							<v-row no-gutters>
								...
								<v-col xs="12" sm="6" md="6">
									<v-card flat>
										...
										<v-card-subtitle>
											{{
												this.$store.getters["uiadmin/getProdiName"](
													data_transaksi.kjur
												)
											}}
										</v-card-subtitle>
						...
						>
							<template v-slot:top>
								<v-toolbar flat color="white">
									<v-toolbar-title>
										DAFTAR BULAN SPP YANG AKAN DIBAYAR
									</v-toolbar-title>
									...
									<v-btn
										...
										:disabled="
											data_transaksi.status == 1 || data_transaksi.status == 2
										"
									>
							...
							<template v-slot:no-data>
								daftar bulan yang akan dibayar belum tersedia; silahkan pilih
								bulan di bawah ini.
							</template>
						</v-data-table>
					</v-form>
	...
	export default {
		...
		methods: {
			...
			initialize: async function() {
				...
				await this.$ajax
					.get("/keuangan/transaksi-spp/" + this.transaksi_id, {
						headers: {
