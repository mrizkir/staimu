		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman ini digunakan untuk memperoleh laporan penerimaan Daftar Ulang
					Mahasiswa Baru.
				</v-alert>
			</template>
		</ModuleHeader>
						...
						>
							<div class="v-toolbar__content" style="height: 64px;">
								<div class="v-toolbar__title">
									REKAPITULASI PENERIMAAN DAFTAR ULANG MAHASISWA BARU
								</div>
								<hr
									role="separator"
...
<script>
	...
	export default {
		name: "TransaksiLaporanDulangMHSBaru",
		created() {
			...
			this.breadcrumbs = [
				...
				{
					text: "LAPORAN PENERIMAAN DAFTAR ULANG MAHASISWA BARU",
					disabled: true,
					href: "#",
				},
			];
		...
		methods: {
			...
			initialize: async function() {
				...
				await this.$ajax
					.post(
						"/keuangan/transaksi-laporandulangmhsbaru",
						{
			...
			printtoexcel: async function() {
				...
				await this.$ajax
					.post(
						"/keuangan/transaksi-laporandulangmhsbaru/printtoexcel1",
						{
							ta: this.tahun_akademik,
						},
						{
