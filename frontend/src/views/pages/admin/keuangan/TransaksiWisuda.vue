<template>
  <KeuanganLayout>
    <ModuleHeader>
      ...
      <template v-slot:name> TRANSAKSI WISUDA </template>
      ...
      <template v-slot:desc>
        <v-alert color="cyan" border="left" colored-border type="info">
          Halaman ini digunakan untuk melakukan transaksi WISUDA mahasiswa
          lama.
        </v-alert>
      </template>
    </ModuleHeader>
        ...
        <v-col cols="12">
          ...
          >
                ...
                <v-dialog v-model="dialogfrm" max-width="500px" persistent>
                  <v-form ref="frmdata" v-model="form_valid" lazy-validation>
                    <v-card>
                      <v-card-title>
                        <span class="headline"
                          >TAMBAH TRANSAKSI T.A {{ tahun_akademik }}</span
                        >
                      </v-card-title>
                      <v-card-text>
                        <v-text-field
            ...
            <template v-slot:no-data>
              Data transaksi WISUDA belum tersedia
            </template>
          </v-data-table>
        </v-col>
      </v-row>
    </v-container>
    <dialog-printout pid="WISUDA" title="WISUDA" ref="dialogprint"></dialog-printout>
  </KeuanganLayout>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    viewItem(item) {
      this.$router.push("/keuangan/transaksi-wisuda/" + item.transaksi_id);
    },
    save: async function() {
      if (this.$refs.frmdata.validate()) {
        ...
        await this.$ajax
          .post(
            "/keuangan/transaksi-wisuda/store",
            {
    ...
    deleteItem(item) {
      this.$root.$confirm
        .open(
          "Delete",
          "Apakah Anda ingin menghapus data transaksi wisuda dengan ID " +
            item.id +
            " ?",
        ...
        .then(confirm => {
          if (confirm) {
            ...
            this.$ajax
              .post(
                "/keuangan/transaksi-wisuda/" + item.transaksi_id,
                {
                  _method: "DELETE",
                },
                {
