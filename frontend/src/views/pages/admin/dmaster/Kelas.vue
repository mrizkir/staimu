		...
		methods: {
			...
			save: async function() {
				if (this.$refs.frmdata.validate()) {
					...
					if (this.editedIndex > -1) {
						await this.$ajax
							.post(
								"/datamaster/kelas/" + this.old_idkelas,
								{
									_method: "PUT",
			...
			deleteItem(item) {
				this.$root.$confirm
					.open(
						"Delete",
						"Apakah Anda ingin menghapus data dengan ID " + item.idkelas + " ?",
						{ color: "red" }
					)
					.then(confirm => {
						if (confirm) {
