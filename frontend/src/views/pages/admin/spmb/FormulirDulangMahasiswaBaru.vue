<template>
	<SPMBLayout>
		<ModuleHeader>
			...
			<template v-slot:desc v-if="dashboard=='mahasiswabaru'">
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman ini berisi pengisian formulir daftar ulang, mohon diisi dengan lengkap dan benar.
				</v-alert>
			</template>
		...
		<v-row class="mb-4" no-gutters>
			<v-col cols="12" v-if="dashboard == 'mahasiswabaru' && formdata.isdulang == false">
		...
		methods: {
			...
			save: async function()
			{
				if (this.$refs.frmdata.validate())
				{
					...
					await this.$ajax.post("/akademik/dulang/mhsbaru/" + this.$store.getters["auth/AttributeUser"]("id"), {
						_method: "put",
