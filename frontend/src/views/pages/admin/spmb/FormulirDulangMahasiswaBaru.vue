		...
		<ModuleHeader>
			...
			<template v-slot:desc v-if="dashboard == 'mahasiswabaru'">
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman ini berisi pengisian formulir daftar ulang, mohon diisi dengan
					lengkap dan benar.
				</v-alert>
			</template>
...
<script>
	...
	export default {
		name: "FormulirDulangMahasiswaBaru",
		created() {
			...
			this.breadcrumbs = [
				...
				{
					text: "FORMULIR DAFTAR ULANG MAHASISWA BARU",
					disabled: true,
					href: "#",
				},
			];
					...
					.then(({ data }) => {
						if (data.status == 1) {
							if (data.nilai.ket_lulus == 1) {
								...
								if (this.peserta.isdulang) {
									this.errorMessage = "Mahasiswa ini telah melakukan daftar ulang mahasiswa baru.";
								} else {
									this.open_form = true;
								}
							} else {
								this.errorMessage = "Mahasiswa ini hasil ujian PMB-nya dinyatakan tidak lulus.";
							}
						} else {
							this.errorMessage = "Mahasiswa ini belum melakukan ujian PMB.";
						}
					});
			},
			save: async function() {
