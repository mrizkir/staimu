<template>
	<SystemUserLayout>
		...
		<v-container fluid>
			...
			<v-row>
				<v-col xs="12" sm="6" md="6" v-if="dashboard == 'mahasiswa'"></v-col>
				<v-col xs="12" sm="6" md="6">
					<v-form
						ref="frmuploadfoto"
		...
		data: () => ({			
			...
			rule_foto: [
				...
				value =>
					!!value ||
					value.size < 2000000 ||
					"File foto harus kurang dari 2MB.",
			],
			rule_user_password: [
			...
			rule_foto_profil: [
				...
				value =>
					!!value ||
					value.size < 2000000 ||
					"File Foto Profil harus kurang dari 2MB.",
			],
		}),
		methods: {
			save() {
			...
			uploadFoto: async function() {
				if (this.$refs.frmuploadfoto.validate()) {
					if (this.formdata.foto) {
						...
						await this.$ajax
							.post(
								"/setting/users/uploadfoto/" + this.$store.getters.User.id,
								formdata,
								{
									headers: {
										Authorization: this.$store.getters["auth/Token"],
			...
			resetFoto: async function() {
				...
				await this.$ajax
					.post(
						"/setting/users/resetfoto/" + this.$store.getters.User.id,
						{},
						{
							headers: {
								Authorization: this.$store.getters["auth/Token"],
			...
			async fetchMahasiswa() {
				await this.$ajax
					.get(
						"/akademik/kemahasiswaan/biodatamhs1/" +
							this.$store.getters["auth/AttributeUser"]("id"),
						{
							headers: {
								Authorization: this.$store.getters["auth/Token"],
