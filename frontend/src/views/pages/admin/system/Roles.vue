						...
						<template v-slot:top>
							<v-toolbar flat color="white">
								...
								>
									<RolePermissions
										:role="editedItem"
										:daftarpermissions="daftar_permissions"
										:permissionsselected="permissions_selected"
										v-on:closeRolePermissions="closeRolePermissions"
									/>
								</v-dialog>
							</v-toolbar>
						</template>
								...
								<v-row no-gutters>
									...
									<v-col xs="12" sm="6" md="6">
										<v-card flat>
											...
											<v-card-subtitle>
												{{
													$date(editedItem.created_at).format(
														"DD/MM/YYYY HH:mm"
													)
												}}
											</v-card-subtitle>
								...
								<v-row no-gutters>
									...
									<v-col xs="12" sm="6" md="6">
										<v-card flat>
											...
											<v-card-subtitle>
												{{
													$date(editedItem.updated_at).format(
														"DD/MM/YYYY HH:mm"
													)
												}}
											</v-card-subtitle>
...
<script>
	...
	export default {
		...
		methods: {
			...
			setPermission(item) {
				this.$ajax
					.get("/system/setting/permissions", {
						headers: {
							Authorization: this.TOKEN,
						},
					})
			...
			save() {
				...
				if (this.$refs.frmdata.validate()) {
					...
					if (this.editedIndex > -1) {
						this.$ajax
							.post(
								"/system/setting/roles/" + this.editedItem.id,
								{
									_method: "PUT",
