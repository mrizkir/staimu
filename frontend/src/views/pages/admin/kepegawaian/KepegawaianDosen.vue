		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Halaman ini untuk melakukan pengelolaan data dosen. Namun untuk
					menghapus dan menambah di halaman user dosen.
				</v-alert>
			</template>
		</ModuleHeader>
		<v-container fluid>
	...
	export default {
		...
		data: () => ({
			...
			rule_user_name: [
				...
				value =>
					/^[A-Za-z\s]*$/.test(value) ||
					"Nama Dosen hanya boleh string dan spasi",
			],
			rule_nidn: [value => /^[0-9]+$/.test(value) || "NIDN hanya boleh angka"],
			rule_nipy: [
				value =>
					/^[0-9]+$/.test(value) ||
					"Nomor Induk Pegawai Yayasan (NIPY) hanya boleh angka",
			],
			rule_user_email: [
		...
		methods: {
			...
			editItem: async function(item) {
				this.$ajax
					.get("/datamaster/jabatanakademik", {
						headers: {
							Authorization: this.TOKEN,
						},
					})
			...
			save() {
				if (this.$refs.frmdata.validate()) {
					...
					if (this.editedIndex > -1) {
						this.$ajax
							.post(
								"/kepegawaian/dosen/" + this.editedItem.id,
								{
									_method: "PUT",
